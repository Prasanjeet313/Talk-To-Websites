...
def main():
        ...
        if st.button("🔄 Load Webpage", type="primary"):
            with st.spinner("Processing..."):
                ...
                if result['success']:
                    ...
                    st.info(f"📏 **Content:** {result['content_length']:,} chars")
                else:
    ...
    if st.session_state.webpage_loaded:
            ...
            with st.chat_message(msg["role"]):
                ...
                if "sources" in msg and msg["role"] == "assistant":
                    with st.expander("📚 View Sources"):
                        for i, doc in enumerate(msg["sources"][:3], 1):
        ...
        **For Cloudflare-protected sites (recommended):**
        1. Run `selenium_extractor.py` locally to extract webpage data
        2. Upload the generated `webpage_cache.json` to your project
        3. The app will automatically use cached data
        
        **For regular sites:**
        - Simply enter the URL and click "Load Webpage"
        - The app will attempt to scrape directly
        
        ### 💡 Benefits:
        - ✅ Bypass Cloudflare protection using local Selenium extraction
        - ✅ Faster loading from pre-extracted cache
        - ✅ Share extracted data across deployments
